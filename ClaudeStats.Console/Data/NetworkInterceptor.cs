...
/// </summary>
public sealed class NetworkInterceptor(bool discoverMode = false)
{
    ...
    public IReadOnlyList<(string Url, string ContentType, string Preview)> DiscoveredResponses => _discovered.AsReadOnly();
    ...
    public void Register(IPage page)
    {
        page.Response += async (_, response) =>
        {
            try
            {
                ...
                // Skip static assets
                if (url.EndsWith(".js") || url.EndsWith(".css") || url.EndsWith(".ico") || url.EndsWith(".png") || url.EndsWith(".svg") || url.EndsWith(".woff2"))
                {
                    return;
                }
    ...
    public async Task<InterceptedData> WaitForDataAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            overageLimitJson = await _overageLimitTcs.Task.WaitAsync(graceCts.Token);
        }
        catch
        {
            /* optional */
        ...
        try
        {
            creditGrantJson = await _creditGrantTcs.Task.WaitAsync(graceCts.Token);
        }
        catch
        {
            /* optional */
        ...
        try
        {
            prepaidCreditsJson = await _prepaidCreditsTcs.Task.WaitAsync(graceCts.Token);
        }
        catch
        {
            /* optional */
